classdef WhiskingFun
    methods(Static)
        ...
        %% FindPeakWhisking
        %%%%%%%%%%%%%%%%%%%
        function peakWhisking=FindPeakWhisking(periodBehavData_ms)
            ...
            for whiskerTraceNum=1:size(periodBehavData_ms,1)
                peakWhisking(whiskerTraceNum,:)=[0 0 diff(cummax(abs(diff(periodBehavData_ms(whiskerTraceNum,:)))))];
        ...
        %% FindWhiskingEpochs
        %%%%%%%%%%%%%%%%%%%%%
        function [whiskingEpochsIdx,wAmplitude,setPoint]=...
                FindWhiskingEpochs(whiskerTraces,whiskingPhase,ampThreshold,minDur)
            ...
            %ampThreshold = 20 ;  % (degrees) threshold on whisker position amplitude above which that part of a recording is taken to be a whisking bout
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function LP_periodBehavData=LowPassBehavData(periodBehavData,samplingRate,threshold)
            if nargin==1
                samplingRate=1000;
            elseif nargin==2
                threshold=20;
            ...
            for whiskerTraceNum=1:size(periodBehavData,1)
                LP_periodBehavData(whiskerTraceNum,:)=...
        ...
        function BP_periodBehavData=BandPassBehavData(periodBehavData,samplingRate,threshold)
            if nargin==1
            ...
            for whiskerTraceNum=1:size(periodBehavData,1)
                BP_periodBehavData(whiskerTraceNum,:)=...
        ...
        function HP_periodBehavData=HighPassBehavData(periodBehavData,samplingRate,threshold)
            if nargin==1
                samplingRate=1000;
            elseif nargin==2
                threshold=0.3;
            ...
            for whiskerTraceNum=1:size(periodBehavData,1)
                HP_periodBehavData(whiskerTraceNum,:)=...
        ...
        %% Calculate Phase
        %%%%%%%%%%%%%%%%%%
        function [whiskingPhase,protractionIdx,retractionIdx]=ComputePhase(periodBehavData,whiskingPeriodIdx)
            % See Hill et al. 2011
            if nargin==1 %% compute phase over whole trace
                whiskingPeriodIdx=ones(1,size(periodBehavData,2));
            end
            [whiskingPhase,protractionIdx,retractionIdx]=deal(nan(size(periodBehavData)));
            for whiskerTraceNum=1:size(periodBehavData,1)
                ...
                whiskingPhase(whiskerTraceNum,:)=angle(HTangleTrace);
                % identify protraction and retraction epochs
                protractionIdx(whiskerTraceNum,:)=whiskingPhase(whiskerTraceNum,:)<0;
                retractionIdx(whiskerTraceNum,:)=whiskingPhase(whiskerTraceNum,:)>0;
            end
            whiskingPhase(:,~whiskingPeriodIdx)=nan;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function instantFreq=ComputeInstFreq(periodBehavData)
            Nfft = 1024;
