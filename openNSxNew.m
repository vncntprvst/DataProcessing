function varargout = openNSxNew(varargin)
    ...
    elseif strfind(' hour min sec sample ', [' ' inputArgument ' ']) ~= 0
        ...
        if length(temp)>3 && strcmpi(temp(end-3),'.')
            ...
            if exist(fname, 'file') ~= 2
                disp('The NSx file does not exist.');
                if nargout; 
                    varargout{1} = []; 
                end
                return;
...
FinalDataHeader=DataHeader';
FinalDataHeader=FinalDataHeader(1:headerCount); %in case size is different
NSx.RawData.DataHeader = cat(1,FinalDataHeader{:});
